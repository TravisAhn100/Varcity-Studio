...
export type Config = {construction:Construction;fit:'regular';size:string;mannequin:'none'|'male'|'female';fabricCollar?:Choice} & Record<Region,Choice>;
...
export const changeConstruction=(config:Config,patch:Partial<Construction>):Config=>({...config,construction:{...config.construction,...patch}});
export const constructionLabel=(c:Construction)=>`${c.shoulder==='raglan'?'Raglan':'Regular'} · ${c.closure==='placket'?'Zip + placket':c.closure==='zipper'?'Zipper':'Snaps'} · ${c.collar==='high-neck'?'High neck':'Varsity'}`;
export const hardwareLabel=(c:Construction)=>c.closure==='snaps'?'Snaps':c.closure==='zipper'?'Zipper / hardware':'Zipper & snaps';
